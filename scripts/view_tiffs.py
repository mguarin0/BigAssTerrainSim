...
# Print basic info
print(f"Shape: {heightmap.shape}, Min Elevation: {np.min(heightmap)}, Max Elevation: {np.max(heightmap)}")
...
heightmap_norm = (255 * (heightmap - np.min(heightmap)) / (np.max(heightmap) - np.min(heightmap))).astype(np.uint8)
